...
func readDirectoryHeader(f *File, r io.Reader) error {
	...
	if len(f.Extra) > 0 {
		...
		for len(b) >= 4 { // need at least tag and size
			...
			switch tag {
			case zip64ExtraId:
				// The ZIP64 extra block holds ONLY the fields whose 32-bit value
				// is the 0xFFFFFFFF sentinel, in this order: uncompressed size,
				// compressed size, local-header offset. Upstream read them
				// unconditionally by remaining length, so a member with small
				// sizes but a >4GB offset (extra = just the 8-byte offset) had its
				// offset read into UncompressedSize64 and headerOffset left at the
				// 0xFFFFFFFF sentinel -> seek to ~4GB -> "not a valid zip file".
				// Mirror archive/zip: consume each 8-byte field only when its base
